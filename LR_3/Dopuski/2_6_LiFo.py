...
class SystemMD1:
    ...
    def __modelling(self):
        ...
        while i_req < self.__number_expiriences or stack:
            if not stack and i_req < self.__number_expiriences:
                current_time = self.__global_time_requests_in[i_req]
                stack.append(i_req)
                i_req += 1

            while (i_req < self.__number_expiriences and
                   self.__global_time_requests_in[i_req] <= current_time + 1):
                stack.append(i_req)
                i_req += 1

            if stack:
                req_index = stack.pop()
                start_service = max(current_time, global_time_out)

                if self.__sync_mode:
                    start_service = math.ceil(start_service)

                end_service = start_service + service_time

                self.__global_time_requests_out[req_index] = end_service
                self.__req_wait_time[req_index] = end_service - self.__global_time_requests_in[req_index]

                if self.__lambda > 0.8:
                    print(f"Заявка {req_index}; t_out={end_service:>3f}; d={self.__req_wait_time[req_index]:>3f}")

                global_time_out = end_service
                current_time = end_service
                i_out += 1

        if self.__sync_mode:
            self.__sync_calculate_mean_N()
        else:
...
if __name__ == "__main__":
    number_experiences = 250_000

    count_points = 21
    # lambdas = [0.99]
    lambdas = np.linspace(0.9, 0.98, count_points)
    ...
    plt.figure()
    plt.title(f"LIFO. Среднее число абонентов в системе для СМО M|D|1")
    plt.xlabel(f"$\lambda$")
